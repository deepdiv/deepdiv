...
# 对外提供一个方法,用于实验3
#
def get_len_data(data_name, dau_name):
    # 1. 获得原始数据
    X_train, X_test, Y_train_vec, Y_test_vec = data_gener.gen_ori_data(data_name)
    Y_train = np.argmax(Y_train_vec, axis=1)
    len_test = len(X_test)
    # 2. 获得扩增数据
    Xa_train, _, Ya_train, _ = data_gener.gen_dau_data(dau_name, use_norm=True, num=train_dau_num)
    _, Xa_test, _, Ya_test = data_gener.gen_dau_data(dau_name, use_norm=True, num=test_dau_num)
    # 2. 混洗训练集,混洗扩增数据:
    np.random.seed(42)
...
# 本次使用的覆盖方法
def get_cov_name_and_func(cover):
    cal_nac_cov75 = functools.partial(cover.cal_nac_cov, t=0.75)
    cal_tknc_cov1 = functools.partial(cover.cal_tknc_cov, k=1)
    # cov_name_list = ["LSC", ]  # "DSC",
    # func_list = [cover.cal_lsc_cov]  # cover.cal_dsc_cov, # lsc

    # cov_name_list = ["DeepGini", "NAC", "NBC", "SNAC", "KMNC", "TKNC", "LSC", ]  # "DSC",
    # func_list = [cover.cal_deepgini, cal_nac_cov75, cover.cal_nbc_cov, cover.cal_snac_cov, cover.cal_kmnc_cov,
    #              cal_tknc_cov1, cover.cal_lsc_cov]  # cover.cal_dsc_cov, # lsc

    cov_name_list = ["NAC", "NBC", "SNAC", "TKNC", "LSC"]  # "DSC",
    func_list = [cal_nac_cov75, cover.cal_nbc_cov, cover.cal_snac_cov, cal_tknc_cov1,
                 cover.cal_lsc_cov]  # kmnc和dsc 使用其他实验文件
...
def get_div_data(Xa_train, Ya_train, base_path, select_size, b, ub, select_pnum=None, is_anlyze=False,
    ...
    csv_data = get_ps_csv_data()
    # b_list, ub_list = [0, ], [0.99]  # 不分块  直接进行选取
    # b_list, ub_list = [0.5, ], [0.99]  # 不分块  只选取高置信度部分,低的不要
    # b = b_list[0]
    # ub = ub_list[0]
    # 计算div
    # 设定参数&计算数据
    ...
    s = time.time()
    Xr_select, select_lable_arr, c_arr, max_size_arr = deepdiv.datasets_select(Xa_train, Ya_train, params["nb_classes"],
                                                                               ori_model,
                                                                               target_size,
...
# 根据下标去获得数据
def get_retrain_data_by_idx(Xa_train, Ya_train, idx, ):
...
def get_random_retrain_data(df, select_size, is_val_data=False, verbose=True):
...
############
############
def exp_retrain(base_path, select_size, cov_name_list=None,
                verbose=True, is_retrain_cov=True, is_retrain_div=True,
...
def exp(params):
    ...
    is_retrain_div = True  # 是否重训练div
    is_retrain_cov = True  # 两个false就只重训练div
    is_retrain_random = True
    base_path = None  # 如果不准备ps,直接retrain,就填写ps的路径,
    # base_path = "./result/11261825_mix2dau_mnist_LeNet5/"  # 如果不准备ps,直接retrain,就填写ps的路径,
    verbose = 2  # 打印模型训练信息
    # 1 获得原始数据
    ...
    # cov_name_list = ["DeepGini"]
    # func_list = [cover.cal_deepgini]
    if is_prepare_ps:
        # 5. 创建实验文件夹
        df = None
        ...
        for k in exp_params['k_list']:
            ...
            if is_retrain:
                # 重训练
                exp_retrain(base_path, select_size, cov_name_list=cov_name_list, verbose=verbose,
                            is_retrain_cov=is_retrain_cov,
                            is_retrain_random=is_retrain_random, is_retrain_div=is_retrain_div)
    statistic_data(base_path, exp_params["k_list"], len(X_select))
    # if is_retrain:
    #     for k in exp_params['k_list']:
    #         select_size = int(len(Xa_train) * k)
    #         # 重训练
    #         exp_retrain(base_path, select_size, cov_name_list=cov_name_list, ttrs=temp_train_size, ttts=temp_test_size,
    #                     is_val_data=is_val_data,
    #                     is_val_retrain=is_val_retrain, verbose=verbose, is_retrain_cov=is_retrain_cov,
    #                     is_retrain_random=is_retrain_random, is_retrain_div=is_retrain_div)
...
def set_params(p=None, ename=None, train_dn=1, test_dn=1):
    global params
