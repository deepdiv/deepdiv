...
# 本次使用的覆盖方法
def get_cov_name_and_func(cover, max_select_size):
    cal_dsc_cov_limit = functools.partial(cover.cal_dsc_cov, time_limit=36000)
    cov_name_list = ["DSC"]  # "DSC",
    func_list = [cal_dsc_cov_limit]
...
def get_retrain_idx(cam_path, ctm_path, select_size, Xa_train_size):
    ...
    if os.path.exists(cam_path):
        ...
        if len(cam_ps_arr) >= select_size:
            ...
            # else:
            #     # 从ctm里选
            #     cam_idx = list(cam_ps_arr)
            #     for idx in ctm_ps_arr:
            #         if idx not in cam_ps_arr:  # 在ctm里有但是在cam里没有的
            #             cam_idx.append(idx)  # 添加到cam里
            #             if len(cam_idx) == select_size:
            #                 break
...
def get_random_retrain_data(df, select_size, verbose=True):
    ...
    acc_imp_ori, acc_imp_val, retrain_time = get_retrain_res(x_s, y_s, verbose=verbose)  # 增加训练后模型与数据分析
    ...
    if is_retrain_cov:
        ...
        for name in cov_name_list:
            ...
            for k, idx in temp_idx_data.items():
                ...
                acc_imp_ori, acc_imp_val, retrain_time = get_retrain_res(x_s, y_s, verbose=verbose)
...
def exp(params):
    ...
    cov_name_list, func_list = get_cov_name_and_func(cover, max_select_size)
    if is_prepare_ps:
        # 5. 创建实验文件夹
        df = None
